...
void Logo::Init()
{
	...
	for (int i = 0; i < 420; i++)
	{
		sprintf_s(FileName, "./resource/Img/opening/opening%03d.png", i);
		Opening[i].Create(FileName, false, D3DCOLOR_XRGB(0, 0, 0));
	}
...
void Logo::Update(double frame)
{
	...
	//오프닝 해제할것
	if (GetTickCount() - OpeningTime > 46)//기준 값 46 숫자가 작을 수록 빠르게 스킵
	{
		m_OpeningCount++;
		if (m_OpeningCount == 1) 
			EffectPlay(sound.Intro);
		if (m_OpeningCount == 340)
			BGPlay(sound.BGM_Menu);
		

		if (m_OpeningCount == 419) 
		{
			g_Mng.n_Chap = MENU;
		}
		OpeningTime = GetTickCount();
	}
...
{
	//오프닝 해제할것
	Opening[m_OpeningCount].SetColor(255,255,255, alpha);  // 색상 변경
	Opening[m_OpeningCount].Render( 0, 0, 0, 1, 1.1 ) ;  //이미지출력
